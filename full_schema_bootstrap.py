...
# build_churn_daily -- the CONVERGENCE LEADING INDICATOR over build_provenance.
# Per day, share of produced files that are CHURN (path rework / version bump /
# glue) vs NET-NEW. Falling churn_pct => converging; rising => plateau risk.
# Ghost builds (no output_path) excluded from the ratio. CREATE OR REPLACE VIEW
# (idempotent). Mirrors schema/builder.duckdb.sql exactly.
ex("""
CREATE OR REPLACE VIEW build_churn_daily AS
WITH produced AS (
    SELECT build_id, built_at::DATE AS day, success, rescue_count, output_path,
...
churned AS (
    SELECT p.*, (fd.first_day < p.day) AS is_path_rework,
           ((fd.first_day < p.day) OR p.is_versioned OR p.is_glue) AS is_churn
    FROM produced p JOIN first_day fd ON fd.output_path = p.output_path
)
SELECT day,
...
FROM churned GROUP BY day ORDER BY day DESC
""", 'build_churn_daily (view)')

# build_churn_trend -- single-row VERDICT: this week's churn vs last week's +
# a regime label (CONVERGING | FLAT | PLATEAU-RISK | BASELINE | NO-DATA).
ex("""
CREATE OR REPLACE VIEW build_churn_trend AS
SELECT cur.churn_pct_7d, prev.churn_pct_7d AS churn_pct_prev_7d,
       ROUND(cur.churn_pct_7d - prev.churn_pct_7d, 1) AS delta_pts,
       CASE WHEN cur.churn_pct_7d IS NULL THEN 'NO-DATA'
...
FROM (SELECT ROUND(100.0 * SUM(churn_files) / NULLIF(SUM(produced_files), 0), 1) AS churn_pct_7d
