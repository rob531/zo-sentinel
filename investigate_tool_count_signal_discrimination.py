#!/usr/bin/env python3
"""
investigate_tool_count_signal_discrimination.py
...
def run_investigation() -> dict:
    print("=" * 70)
    print("tool_count signal discrimination investigation")
    ...
    findings: dict = {
        ...
        "evidence_samples": [],
        "root_cause": None,
        "root_cause_type": None,   # "producer_bug" | "data_quality" | "enrichment_needed"
        "source_module": None,
    ...
    # - If the evidence blobs show varying tool_count values BUT all map to
    #   only 2 score buckets → producer bug (score_tool_count collapses values)
    # - If the evidence blobs all show the same tool_count value → data quality
    # - Enrichment is needed if the producer can't be fixed or the spec calls for it
    ...
    if len(ev_values) > 2 and findings["distinct_values_found"] == 2:
        ...
        findings["fix_recommendation"] = (
            ...
            "modules (tool_count_enrichment_v3.py, tool_description_safety_enrichment.py) "
            "demonstrate correct graduated scoring but are NOT wired into signal_analyser_v2.py. "
            "Either wire the existing enrichment modules into the signal pipeline OR "
            "rewrite score_tool_count() with fine-grained buckets (e.g., 0→25, 1→50, "
            "2→62, 3→71, 4→79, 5→85, 6-10→70-60, 11-20→55-45, 21-50→40-20, 50+→15)."
        )
        ...
        findings["fix_recommendation"] = (
            ...
            "in context_efficiency_enrichment.py and tool_count_enrichment_v3.py "
