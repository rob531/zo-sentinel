...
def _format_report(diag: dict[str, Any]) -> str:
    ...
    if diag["table_exists"] and diag["schema_valid"] and diag["row_count"] == 0:
        lines.append("  Row count:                0 — TABLE EMPTY (pipeline not writing)")
    else:
        lines.append(f"  Row count:                {diag['row_count']}")
    ...
    if not diag["table_exists"]:
        lines.append("")
        lines.append("  STATUS: TABLE MISSING — mcp_signal_enrichments not found in DB")
    elif diag["row_count"] == 0:
        lines.append("")
        lines.append("  STATUS: TABLE EMPTY — pipeline is not writing rows")

    if not diag["table_exists"]:
        lines.append("")
        lines.append("  STATUS: TABLE MISSING — mcp_signal_enrichments not found in database")
    if not diag["table_exists"]:
        lines.append("")
        lines.append("  STATUS: TABLE MISSING — mcp_signal_enrichments not found in database")
    if diag["errors"]:
        ...
        for err in diag["errors"]:
            lines.append(f"    - {err}")
    elif diag["table_exists"] and diag["row_count"] == 0:
        lines.append("")
        lines.append("  STATUS: table is EMPTY — pipeline is not writing rows")
    else:
        lines.append("")
