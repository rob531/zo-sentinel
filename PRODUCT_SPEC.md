...
## PHASE refill 2026-09-20T13:30Z (directive candidates, NOT YET BUILT)

*Refilled by daily-chairman-review (ACT-AUTHORITY: refill a starved anchor) after the deterministic seeder reported the gaps map EXHAUSTED on a CURRENT checkout. The exhaustion reading is trustworthy this time and the distinction matters: at 13:20:06Z the seeder reported EXHAUSTED while the runtime checkout was HEAD=0eddb3a9 != origin/main, which its own message flags as "the anchor may NOT be spent; this checkout may simply predate the refill". safe_ff.sh was fired (0eddb3a9 -> 76aec0529 == origin/main), both goose daemons reloaded on the fresh checkout, and the seeder re-reported at 13:25:12Z with "HEAD=76aec052 == origin/main -- the checkout IS current, so the anchor is genuinely spent". goose_runner had logged "No eligible directives found" on every cycle since its 09:2xZ restart and directives/done/ had not advanced since 2026-09-19 14:57Z (~22.5h idle, short of the 24h factory-dead alarm).*

*Targets chosen from ONE live measurement rather than from prose: `GET https://mcplookup.app/spine/health` at 2026-09-20T13:29Z returned `service_count=33, mounted=30, failures=[], skipped_no_router=["entity_report_exporter","org_api_key_manager","verdict_watchlist_service"]`. All three modules EXIST in git at repo root and each defines its business-logic class but NO module-level `router`, so the spine imports them, finds nothing to mount, and skips. They are registered organs that cannot be reached -- the exact opposite of spineful emission, and they have been invisible because a skipped mount is not counted as a failure. Building three routers moves mounted 30 -> 33 and skipped 3 -> 0 on a prod surface that already publishes the count, so the acceptance test for this refill is a live HTTP read, not a merge count.*

- directive candidate: `entity_report_exporter.py` -- Give the EXISTING `EntityReportExporter` class in this module a module-level FastAPI `router` so the spine can mount it (today: registered in services/active/entity_report_exporter/service.toml with import_path "entity_report_exporter", imported by the spine, skipped for having no `router`). Expose the exporter's report generation as GET returning the export payload plus `{generated_at, entity_id, format}`; keep the existing class as the logic layer and add only the thin router on top -- do NOT rewrite the class. `prefix` and `tag` in service.toml are currently empty strings; set them to "/api" and "entity" in the same change-set so the route does not land at the app root. Postgres-portable SQL, no network in self-test. Exemplar: `verdict_breakdown_api` (services/active/verdict_breakdown_api/router.py -- the mounted sibling with the identical service.toml shape). ACCEPTANCE: contract asserts `from entity_report_exporter import router` succeeds, that `router` is an `APIRouter` with >=1 route, that the route returns 200 over an in-memory SQLite fixture, and that the response carries `generated_at`; prints PASS. [anchor-refill: chairman-review 2026-09-20; basis /spine/health skipped_no_router]
- directive candidate: `org_api_key_manager.py` -- Give the EXISTING `APIKeyManager` class in this module a module-level FastAPI `router` so the spine can mount it (same skipped_no_router cause as above). Expose key LISTING and key METADATA only -- `{key_id, org_id, label, created_at, last_used_at, revoked}` -- and never the key material or its hash; key CREATION and REVOCATION are authenticated mutations and are OUT OF SCOPE for this directive, which is read-only. service.toml `auth` is currently "public": set it to the authenticated value the sibling org surfaces use in the same change-set, since an org's key inventory is not a public surface. Set `prefix`="/api", `tag`="org". Postgres-portable SQL, no network in self-test. Exemplar: `org_risk_summary` (the mounted org-scoped sibling). ACCEPTANCE: contract seeds two orgs with two keys each in in-memory SQLite, asserts 200, asserts only the requesting org's keys are returned, asserts NO field in the response contains the key material or its hash (assert on the actual response dict, not on a doc comment), and asserts a revoked key is present with `revoked=true` rather than silently dropped; prints PASS.
- directive candidate: `verdict_watchlist_service.py` -- Give the EXISTING `VerdictWatchlistService` class in this module a module-level FastAPI `router` so the spine can mount it (same cause). Expose GET over a caller's watchers -- `{watcher_id, org_id, user_id, server_id, created_at, last_notified_at}` -- and a read-only rollup `{watchers_total, servers_watched, notifications_queued}`. The module already carries `queue_notification(...)` and a `WriteService` class: the router MUST NOT call either. This directive adds a read surface only; queueing a notification from an HTTP route is a mutation and is out of scope. Set `prefix`="/api", `tag`="watchlist". Postgres-portable SQL, no network in self-test. Exemplar: `verdict_breakdown_api`. ACCEPTANCE: contract seeds 3 watchers across 2 orgs in in-memory SQLite, asserts 200, asserts the rollup counts match the seeded rows, asserts a watcher that has never been notified reports `last_notified_at=null` rather than being omitted, and asserts no route in the module invokes `queue_notification` (assert by inspecting the router's endpoint callables, not by comment); prints PASS.
- directive candidate: `spine_mount_reachability_report.py` -- Make "registered but unreachable" a VALUE on the health surface instead of an absence. Today `/spine/health` returns `failures: []` alongside `skipped_no_router: [3 services]`, so the surface reads fully green while three registered organs cannot be reached; that is R6 (unknown is not zero) and R3 (a bucket that went to zero must prove the check RAN) on the one surface the loop is graded by. Emit, per service in the active registry, `{service, import_path, import_ok, has_router, route_count, mounted, verdict in MOUNTED|NO_ROUTER|IMPORT_FAILED|UNKNOWN}` plus totals, and require that `mounted + NO_ROUTER + IMPORT_FAILED + UNKNOWN == service_count` so no service can fall out of the census silently. A service whose import raises is IMPORT_FAILED, a service that imports with no `router` is NO_ROUTER, and an unreadable registry is UNKNOWN -- these are three different claims and must never be summed into one "failures" number. Read-only; mounts a report, never repairs. Exemplar: `dashboard_summary_api`. ACCEPTANCE: __main__ over a synthetic registry of four services -- one mounted, one importable with no `router`, one whose import raises, one unreadable -- asserts each verdict distinctly, asserts the four buckets sum to the declared total, and asserts that the import-raising case is NOT reported as NO_ROUTER; prints PASS. [META CAP satisfied: this class cost product output TODAY -- the three unreachable services were invisible to a green health read until a hand query at 13:29Z]

## PHASE refill 2026-09-21T13:15Z (directive candidates, NOT YET BUILT)

*Refilled by daily-chairman-review (ACT-AUTHORITY: refill a starved anchor) after the deterministic seeder reported the gaps map EXHAUSTED on every cycle today. The exhaustion reading was CHECKED against the stale-checkout caveat the seeder itself raises: the runtime checkout is HEAD=f01d873b, 3 commits behind origin/main=0f5d7ae0, and `git diff --name-only HEAD..origin/main` shows NO spec or roadmap file in that range -- so the missing commits cannot explain the empty gaps map and the anchor is genuinely spent. Factory state at refill time: proposed=0 live, pending=0 against a cap of 40; goose_runner at cycle 499 with 719 of 734 cycles today logging "No eligible directives found"; the architect logged STARVATION FLOOR 48 times.*

*Each target below is a SERVICE UNIT and must be proposed as the service unit, not as a single module: handler=build_service, task=build_service_<stem> where stem is the candidate filename without its extension, output_file empty, description = the full paragraph below (the promoter fans the unit out). Targets were de-duplicated before inclusion against services/staged (1,618), services/active (593), directives/done (2,210), directives/proposed (1,033), 4,328 on-disk module basenames and the spec prose itself; three further candidates were REJECTED by that scan as near-duplicates (never_scored_basis_api collided with nine existing never_scored_* services, evidence_bundle_export_api with staged/signal_evidence_export, signal_abstention_report_api with axis_confidence_service below). Absence of an input is a finding (UNKNOWN), never a blank.*

- directive candidate: `signal_provenance_api.py` -- Service unit (router + logic + service.toml) exposing, per scored server, the PROVENANCE of every axis signal behind its verdict: {axis, signal_id, source_feed in osv|ghsa|nvd|registry|llm, source_url, fetched_at, rule_id, contribution}. This is the defensibility half of the assessment product: today a score can be served with no machine-readable answer to the question "on what basis". Every row carries its basis timestamp and feed (R5); a signal whose source cannot be resolved is reported with source_feed=UNKNOWN and is never omitted and never defaulted to registry. Read-only; computes no scores. Exemplar: `verdict_breakdown_api`. ACCEPTANCE: contract seeds two servers with three axis signals each (one lacking a resolvable source) over in-memory SQLite, asserts 200, asserts every returned signal carries fetched_at and source_feed, asserts the unresolvable signal appears with source_feed=UNKNOWN rather than being dropped, and asserts a server with zero signals returns an empty list rather than 404; prints PASS. [anchor-refill: chairman-review 2026-09-21]

- directive candidate: `axis_confidence_service.py` -- Service unit turning each of the seven risk axes into a confidence-bearing claim instead of a bare label: {axis, label, label_index, confidence, evidence_count, verdict in CONFIDENT|WEAK|INSUFFICIENT}. INSUFFICIENT is the point: 99.47 pct of scored representatives sit in the top two tiers (FU-058), which is equally consistent with a genuinely risky population and with a model that abstains from abstaining. Consume `schemas/risk_axis_mapping_v1.json` VERBATIM for class enums (auth_strength has 4 classes; sample-derived enums have burned this before). An axis with zero evidence rows is INSUFFICIENT with a null confidence, never LOW and never 0.0. Exemplar: `scoring_axis_label_distribution_api`. ACCEPTANCE: contract asserts an axis with five corroborating signals is CONFIDENT, one with a single signal is WEAK, one with zero evidence rows is INSUFFICIENT carrying null confidence rather than 0.0, and that a payload whose class enum is absent from the schema is rejected rather than coerced; prints PASS. [anchor-refill: chairman-review 2026-09-21]

- directive candidate: `coverage_gap_servers_api.py` -- Service unit implementing the `GET /coverage/gap/servers` surface that returns 404 in prod today (verified absent from all 71 paths in the published openapi document, 2026-09-19). It is the only instrument that separates a genuine scoring-coverage gap from the URL-collision artifact, so while it is missing the gap size is structurally UNKNOWN. Return {servers_total, servers_scored, servers_unscored, basis_at, method} where the denominator is DISTINCT SERVER identity, never a raw registry ROW count: the standing `never_scored = registry_rows - scored_servers` figure subtracts a server count from a row count and must never be reproduced here. An unreadable registry yields verdict UNKNOWN. Exemplar: `never_scored_backlog_api`. ACCEPTANCE: contract seeds a registry where four rows collapse to two distinct server URLs of which one is scored, asserts servers_total=2 and servers_unscored=1 (never 3), asserts the response names its own `method`, and asserts an unreadable registry yields UNKNOWN rather than 0; prints PASS. [anchor-refill: chairman-review 2026-09-21]
- directive candidate: `registry_url_collision_report.py` -- Re-measure, at today's corpus size, the single assumption every coverage figure rests on: on 2026-07-14, 80,539 unscored rows collapsed to 65,552 distinct URLs and 100 pct of unscored rows URL-collided with an already-scored row. The corpus has since grown past 500k rows and that measurement has never been repeated, so every "unscored servers" number in circulation inherits a two-month-old collision rate as though it were current. Report {rows_total, distinct_urls, collision_rate_pct, unscored_rows, unscored_rows_colliding_with_scored, basis_at} plus VERDICT in COLLISION_DOMINANT|GENUINE_GAP|MIXED|UNKNOWN. Normalisation is deterministic (scheme + host + path, case-folded host) and is stated in the output; no fuzzy or embedding matching. Exemplar: `dashboard_summary_api`. ACCEPTANCE: __main__ over a synthetic registry of ten rows spanning six distinct URLs with three unscored rows all colliding asserts COLLISION_DOMINANT, asserts a registry with zero collisions yields GENUINE_GAP, and asserts an unreadable input yields UNKNOWN rather than a 0 pct collision rate; prints PASS. [anchor-refill: chairman-review 2026-09-21]

- directive candidate: `rescore_wave_duration_ledger.py` -- Close the measurement hole under the weekly rescore SLA: `moat-rescore-weekly` watches only `breaching_sla` and job liveness, so wave DURATION -- the quantity that actually predicts a breach -- is unmeasured, and has been reconstructed by hand from run ids on at least three occasions (20260804-060703 = 60.5 min; 20260909-014759 = 86.2 min; rising). Write one durable idempotent row per wave: {run_id, started_at, finished_at, duration_sec, rows_scored, newest_scored_at, source}. Because the margin is the wave-over-wave duration delta, a wave whose end cannot be observed is recorded with finished_at=null and duration_sec=UNKNOWN, never with a guessed end. Re-running over the same wave updates in place and adds no duplicate row. Exemplar: `score_run_ledger_writer`. ACCEPTANCE: __main__ asserts a second run over identical input adds no row and changes no duration, asserts an in-flight wave records duration_sec=UNKNOWN rather than 0, and asserts two waves sharing a start minute remain two rows keyed on run_id; prints PASS. [anchor-refill: chairman-review 2026-09-21]

- directive candidate: `scoring_throughput_ledger_api.py` -- Service unit reading the wave-duration ledger and publishing the breach predictor as a queryable surface: {waves: [{run_id, duration_sec, rows_scored, rows_per_min}], duration_delta_sec, projected_line_at, margin_min, verdict in SAFE|TIGHT|BREACH_LIKELY|UNKNOWN}. Compute the window as `(newest_scored_at + 7d) - scheduled_fire`, which depends only on measured values and holds regardless of when the previous wave actually started; a margin derived by subtracting one window from another has produced a wrong published figure twice and must never be used here. Fewer than two ledgered waves yields UNKNOWN, never SAFE. Every emitted number carries its unit and window (R5). Read-only; fires no wave. Exemplar: `dashboard_summary_api`. ACCEPTANCE: contract asserts two waves of rising duration yield a positive duration_delta_sec and BREACH_LIKELY once the delta exceeds the margin, asserts a single ledgered wave yields UNKNOWN rather than SAFE, and asserts the emitted margin carries its unit and window; prints PASS. [anchor-refill: chairman-review 2026-09-21]
- directive candidate: `risk_evidence_drilldown_view.py` -- The UI surface that makes a verdict inspectable by the person who has to act on it: for one server, render its tier, each axis with label + confidence + verdict sourced from `axis_confidence_service`, and the contributing signals with feed and fetched_at sourced from `signal_provenance_api`, each signal row linking to its source_url. An axis reported INSUFFICIENT renders as INSUFFICIENT alongside its evidence count, never as a neutral or default band -- the entire value of the assessment product is that a thin verdict looks thin on screen. Reads those two APIs over HTTP with a stated timeout; a failed fetch renders UNKNOWN with its reason, never an empty panel that reads as clean. Exemplar: `server_compare_dashboard_view.html`. ACCEPTANCE: __main__ renders against a synthetic payload of seven axes (one INSUFFICIENT, one WEAK) and asserts the INSUFFICIENT axis appears with that word and its evidence count, asserts every signal row emits its fetched_at, and asserts an API timeout renders UNKNOWN text rather than an empty section; prints PASS. [anchor-refill: chairman-review 2026-09-21]

- directive candidate: `student_adapter_acceptance_gate.py` -- The gate that `student_adapter_registry` (already active) was built without: the registry SELECTS an adapter (bar_passes=True, else highest composite) but nothing refuses a selection whose bar was never evaluated at all. Reading the sibling-repo leaderboard contract (rob531/zomesh-sentinel-sft, `leaderboard/sentinel_student.json`), emit {adapter_id, branch, composite, bar_passes, bar_evaluated_at, axes_evaluated, verdict in ACCEPT|REJECT|UNEVALUATED} and refuse promotion on UNEVALUATED. bar_passes=False and bar_evaluated_at=null are different claims and must render as different verdicts: an adapter that was never measured is UNEVALUATED, never REJECT and never ACCEPT-by-composite. Consume `schemas/risk_axis_mapping_v1.json` verbatim. Read-only; promotes nothing. Exemplar: `student_adapter_registry`. ACCEPTANCE: __main__ asserts an adapter with bar_passes=True is ACCEPT, one with bar_passes=False is REJECT, one whose bar_evaluated_at is null is UNEVALUATED even when its composite is the highest, and that an absent leaderboard file yields UNEVALUATED rather than an empty ACCEPT; prints PASS. [anchor-refill: chairman-review 2026-09-21]

- directive candidate: `adapter_promotion_ledger_api.py` -- Service unit giving the student-adapter path the audit trail the scoring path already has: one durable row per promotion decision {decided_at, adapter_id, branch, composite, verdict, gate_version, decided_by, superseded_by}, exposed as a read-only history plus the currently-effective adapter. Today, which adapter served a given score is unreconstructable after the fact, so a scoring regression cannot be attributed to an adapter change. A promotion whose deciding gate version cannot be resolved records gate_version=UNKNOWN and is still ledgered: an unattributable decision is worse unrecorded than imperfectly recorded. Read-only; performs no promotion. Exemplar: `verdict_breakdown_api`. ACCEPTANCE: contract seeds three promotions of which the middle is superseded, asserts the effective adapter is the newest non-superseded row, asserts a promotion with an unresolvable gate records gate_version=UNKNOWN and still appears in the history, and asserts an empty ledger returns an empty history with a null effective adapter rather than 404; prints PASS. [anchor-refill: chairman-review 2026-09-21]
