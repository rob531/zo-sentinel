...
router = APIRouter(prefix="/api", tags=["risk_server_history"])
...
@router.get(
    "/risk-server-history/{server_id}/latest",
...
@router.get(
    "/risk-server-history/{server_id}/trend",
...
if __name__ == "__main__":
    ...
    with TestSessionLocal() as sess:
        ...
        # axis scores — unique per (server_id, axis_name, model_version)
        # use distinct axis_name values so all 3 insert without hitting the unique constraint
        ...
        sess.add(McpLlmAxisScore(
            id=2,
            server_id="srv-history-1", axis_name="auth_strength",
            label="MEDIUM", p_top=0.45, model_version="v1", scored_at=t2,
        ))
        sess.add(McpLlmAxisScore(
            id=3,
            server_id="srv-history-1", axis_name="data_privacy",
    ...
    # Test 1: full history
    resp = client.get("/api/risk-server-history/srv-history-1")
    ...
    # Test 2: latest entry
    resp2 = client.get("/api/risk-server-history/srv-history-1/latest")
    ...
    resp3 = client.get("/api/risk-server-history/nonexistent-server")
    ...
    resp4 = client.get("/api/risk-server-history/srv-history-2")
    ...
    resp5 = client.get("/api/risk-server-history/srv-history-2/latest")
    if resp5.status_code != 404:
        print(f"FAIL: expected 404 for latest on empty server, got {resp5.status_code}")
        sys.exit(1)

    # Test 6: summary
    resp6 = client.get("/api/risk-server-history/summary?period_days=7")
    ...
    # Test 7: trend
    resp7 = client.get("/api/risk-server-history/srv-history-1/trend?period_days=7")
